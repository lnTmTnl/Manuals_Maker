...
function SidebarScript(editor) {
  ...
  container.add(
    new UIText(strings.getKey("sidebar/script")).setTextTransform("uppercase")
  )
    ...
    if (scripts !== undefined && scripts.length > 0) {
      ...
      for (let i = 0; i < scripts.length; i++) {
        ;(function (object, script) {
          const name = new UIInput(script.name)
            .setWidth("130px")
            .setFontSize("12px")
          ...
          remove.onClick(function () {
            if (confirm("Are you sure?")) {
              editor.execute(
                new RemoveScriptCommand(editor, editor.selected, script)
              )
            }
          })
