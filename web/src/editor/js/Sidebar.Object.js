...
function SidebarObject(editor) {
  ...
  // Scriptable
  const objectScriptableRow = new UIRow()
  const objectScriptable = new UICheckbox().onChange(update)

  objectScriptableRow.add(
    new UIText(strings.getKey("sidebar/object/scriptable")).setWidth("90px")
  )
  objectScriptableRow.add(objectScriptable)

  container.add(objectScriptableRow)
  ...
  function update() {
    ...
    if (object !== null) {
      ...
      if (
        ...
        objectScriptable.getValue()
      ) {
        if (objectScriptable.getValue() === true) {
          document.scriptableObjects.push(object.uuid)
        } else {
          document.scriptableObjects.splice(
            document.scriptableObjects.indexOf(object),
            1
          )
        }
        editor.execute(
